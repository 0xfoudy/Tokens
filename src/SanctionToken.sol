...
contract SanctionToken is ERC20, IERC1363, ERC165 {
    ...
    function removeAdmin(address addy) public {
        delete _admins[addy];
    }
    ...
    function blackList(address[] memory addies) public onlyAdmin {
        for (uint256 i = 0; i < addies.length; ++i) {
            _blacklisted[addies[i]] = true;
        }
    }

    function unblackList(address addy) public onlyAdmin {
        delete _blacklisted[addy];
    }

    function isBlacklisted(address addy) public view returns (bool) {
        return _blacklisted[addy];
    }
